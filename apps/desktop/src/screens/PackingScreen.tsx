...
interface OrderData {
  ...
  mapped_shipstation_account_ids?: string[];
}
...
export function PackingScreen({ orderId, stationId, onFinish }: Props) {
  ...
  async function initPacking() {
    try {
      ...
      const previewStream = await camera.startPreview();
      if (!previewStream) {
  ...
  async function handleRetryUpload() {
    if (!uploadJobId) return;
    setError('');
    await uploadQueue.retryFailed(uploadJobId);
  }
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      {error && (
        <div className="flex items-center justify-between gap-4 bg-destructive/10 px-6 py-2 text-sm text-destructive">
          <span>{error}</span>
          {uploadJobId && uploadJobStatus === 'failed' && (
            <button
              onClick={handleRetryUpload}
              className="rounded border border-destructive/30 px-3 py-1 text-xs font-semibold text-destructive hover:bg-destructive/5"
            >
              Retry Upload
            </button>
          )}
          ...
          {(status === 'shipping' || status === 'done') && (
            <>
              ...
              <ShippingPanel
                ...
                allowedAccountIds={order?.mapped_shipstation_account_ids}
              />
            </>
          )}
