...
export default async function StatusPage() {
  ...
  const statusChecks = [
    { label: 'Platform health', value: health?.status === 'ok' ? 'Operational' : 'Degraded' },
    {
      ...
      value: health?.checks.supabase === 'ok' ? 'Connected' : 'Unavailable',
    },
    {
      label: 'Maintenance cron',
      value: health?.checks.cron === 'configured' ? 'Configured' : 'Missing',
    },
    {
      label: 'Last health timestamp',
      value: health?.timestamp ? new Date(health.timestamp).toLocaleString() : 'Unavailable',
    },
  ];
    ...
    <div className="min-h-screen bg-muted/30 px-6 py-16">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mt-4 max-w-2xl text-base text-muted-foreground">
          This page summarizes the current live state of the PackagePro deployment and the core user
          flows required for onboarding and fulfillment setup.
        </p>
