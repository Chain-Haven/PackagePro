...
export async function GET(request: NextRequest) {
  try {
    ...
    if (/^[0-9a-f-]{36}$/i.test(scan)) {
      const { data } = await query.eq('id', scan).maybeSingle();
      order = data;
    }

    if (!order) {
      const { data } = await query.eq('woo_order_number', scan).maybeSingle();
      order = data;
    }

    if (!order && /^\d+$/.test(scan)) {
      const { data } = await query.eq('woo_order_id', Number.parseInt(scan, 10)).maybeSingle();
      order = data;
    }
