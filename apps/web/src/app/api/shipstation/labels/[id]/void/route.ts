...
async function handleVoid(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (accountId) {
      ...
      const { data: labelRecord } = await admin
        .from('shipping_labels')
        .select('order_id, org_id, shipstation_account_id')
        .eq('external_label_id', labelId)
        .maybeSingle();

      if (!labelRecord?.order_id || !labelRecord.shipstation_account_id) {
        return NextResponse.json({ error: 'account_id required' }, { status: 400 });
      }

      const access = await requireOrderAccess(labelRecord.order_id as string, request, [
        'org_owner',
        'org_admin',
        'warehouse_manager',
      ]);
      ...
      accountId = labelRecord.shipstation_account_id as string;
      const { data: accountRecord } = await admin
