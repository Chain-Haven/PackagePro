...
export async function POST(request: NextRequest) {
  try {
    ...
    const mappings = Array.isArray(store?.shipstation_store_mappings)
      ? store.shipstation_store_mappings
      : [];
    const mapping = account_id
      ? mappings.find(
          (entry: Record<string, unknown>) => entry.shipstation_account_id === account_id
        )
      : mappings.length === 1
        ? mappings[0]
        : null;
    ...
    if (!account) {
      return NextResponse.json(
        {
          error: account_id
            ? 'Selected ShipStation account is not mapped to this store'
            : mappings.length > 1
              ? 'Select a ShipStation account for this store'
              : 'No ShipStation account configured',
        },
        { status: mappings.length > 1 || account_id ? 409 : 400 }
      );
    }
