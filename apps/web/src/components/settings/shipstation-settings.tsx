...
export function ShipStationSettings({
  ...
  const currentMappingByStore = useMemo(() => {
    const entries = new Map<string, Mapping>();
    mappings.forEach((mapping) => entries.set(mapping.store_id, mapping));
    return entries;
  }, [mappings]);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-border bg-background p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Each WooCommerce store must map to exactly one ShipStation account and ShipStation store
          before desktop label creation is enabled.
        </p>
        <div className="mt-4 space-y-4">
          ...
          {initialStores.map((store) => {
            const currentMapping = currentMappingByStore.get(store.id);
            const mappedAccountId = currentMapping?.shipstation_account_id || accounts[0]?.id || '';
            ...
            return (
              <div key={store.id} className="rounded-xl border border-border p-4">
                <div className="mb-3">
                  ...
                  {currentMapping && (
                    <p className="mt-1 text-xs text-primary">
                      Currently mapped to {currentMapping.shipstation_store_name || currentMapping.shipstation_store_id}
                    </p>
                  )}
                </div>
                ...
                <div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                  ...
                  <select
                    defaultValue={currentMapping?.shipstation_store_id || ''}
                  ...
                  <div className="flex gap-2">
                    ...
                    {currentMapping && (
                      <button
                        type="button"
                        onClick={() => void handleDeleteMapping(currentMapping.id)}
