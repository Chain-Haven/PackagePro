...
export function OnboardingFlow() {
  ...
  useEffect(() => {
    if (!orgSlug && orgName) setOrgSlug(slugify(orgName));
  }, [orgName, orgSlug]);
  useEffect(() => {
    if (currentStep === 'done') {
  ...
  async function handleCreateStore(e: FormEvent) {
    ...
    try {
      const res = await fetch('/api/stores', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ org_id: primaryOrg.id, name: storeName, url: storeUrl }),
      });
      const payload = await res.json();
      if (!res.ok) throw new Error(payload.error ?? 'Failed');
      await loadSetupState();
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-8">
          ...
          <h1 className="mt-3 text-4xl font-extrabold tracking-tight">
            Get your first packing station online
          </h1>
          <p className="mt-3 max-w-2xl text-base text-muted-foreground">
            Create your organization, connect your WooCommerce store, pair the plugin, and register
            a station.
          </p>
        </div>
              ...
              <form className="space-y-5" onSubmit={handleCreateOrg}>
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Top-level account that owns your stores, stations, and videos.
                  </p>
                </div>
            ...
            {!loading && currentStep === 'store' && (
              <form className="space-y-5" onSubmit={handleCreateStore}>
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Enter the URL of the WooCommerce store where you installed the PackagePro
                    plugin.
                  </p>
                </div>
            ...
            {!loading && currentStep === 'pair' && primaryStore && (
              <form className="space-y-5" onSubmit={handlePairPlugin}>
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Open your WooCommerce store admin at <strong>{primaryStore.url}</strong>, go to{' '}
                    <strong>WooCommerce &rarr; PackagePro</strong>, and copy the pairing code shown
                    there. Paste it below.
                  </p>
                </div>
                ...
                <div className="rounded-lg border border-dashed border-primary/30 bg-primary/5 p-5">
                  ...
                  <ol className="space-y-2 text-sm text-muted-foreground">
                    <li>
                      <span className="font-semibold text-foreground">1.</span> Install the
                      PackagePro plugin ZIP on your WooCommerce store
                    </li>
                    <li>
                      <span className="font-semibold text-foreground">2.</span> Go to WP Admin
                      &rarr; WooCommerce &rarr; PackagePro
                    </li>
                    <li>
                      <span className="font-semibold text-foreground">3.</span> Copy the{' '}
                      <strong>pairing code</strong> shown on that page
                    </li>
                    <li>
                      <span className="font-semibold text-foreground">4.</span> Paste it below and
                      click &ldquo;Pair Store&rdquo;
                    </li>
                  </ol>
                </div>
                ...
                <button
                  type="submit"
                  disabled={submitting || pluginCode.length < 4}
            ...
            {!loading && currentStep === 'station' && (
              <form className="space-y-5" onSubmit={handleCreateStation}>
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Name the packing bench so order locks and audit logs stay clear.
                  </p>
                </div>
              ...
              items={[
                {
                  label: primaryOrg
                    ...
                    : 'No organization yet',
                  complete: Boolean(primaryOrg),
                },
                {
                  label: primaryStore
                    ? `${primaryStore.name} — ${storeIsPaired ? 'Paired' : 'Awaiting pairing'}`
                    : 'No store yet',
                  complete: Boolean(primaryStore),
                },
                {
                  label: state.stations[0]
                    ? `${state.stations[0].name} (${state.stations[0].status})`
                    : 'No station yet',
                  complete: state.stations.length > 0,
                },
              ]}
            />
          </div>
      ...
      <ul className="mt-4 space-y-3">
        {items.map((item) => (
          <li key={item.label} className="flex items-center gap-3 text-sm">
            <span
              className={`inline-flex h-6 w-6 items-center justify-center rounded-full border text-xs font-bold ${item.complete ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-muted text-muted-foreground'}`}
            >
              {item.complete ? '✓' : '...'}
            </span>
