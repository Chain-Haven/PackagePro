...
class PackagePro_API {
    ...
    private static function is_pairing_rate_limited($ip) {
        $key = 'packagepro_pairing_' . md5($ip);
        $attempts = get_transient($key);
        return is_array($attempts) && (($attempts['count'] ?? 0) >= self::PAIRING_RATE_LIMIT);
    }

    private static function record_pairing_attempt($ip) {
        $key = 'packagepro_pairing_' . md5($ip);
        $attempts = get_transient($key);
    ...
    public static function handle_pair($request) {
        ...
        if (self::is_pairing_rate_limited($client_ip)) {
        ...
        if (!$pairing_code || !$backend_url || !$store_id) {
            self::record_pairing_attempt($client_ip);
        ...
        $created_at = intval(get_option('packagepro_pairing_code_created_at', 0));

        if (
            !$stored_code
            || !$created_at
            || (time() - $created_at) > self::PAIRING_CODE_TTL_SECONDS
            || strtoupper($pairing_code) !== strtoupper($stored_code)
        ) {
            self::record_pairing_attempt($client_ip);
        ...
        delete_option('packagepro_pairing_code_created_at');
