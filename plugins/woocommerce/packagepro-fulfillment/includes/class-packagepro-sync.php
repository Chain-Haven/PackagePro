...
class PackagePro_Sync {
    private const RECONCILE_BATCH_SIZE = 50;
    private const RECONCILE_ACTION = 'packagepro_reconciliation_batch';
    ...
    public static function init() {
        ...
        add_action(self::RECONCILE_ACTION, [__CLASS__, 'process_reconciliation_batch'], 10, 1);
    }

    public static function run_reconciliation() {
        self::enqueue_reconciliation_batch(0);
    }

    public static function process_reconciliation_batch($offset = 0) {
        ...
        if (count($orders) === self::RECONCILE_BATCH_SIZE) {
            self::enqueue_reconciliation_batch(intval($offset) + self::RECONCILE_BATCH_SIZE);
        }
    }

    private static function enqueue_reconciliation_batch($offset) {
        if (function_exists('as_enqueue_async_action')) {
            as_enqueue_async_action(self::RECONCILE_ACTION, [$offset], 'packagepro');
            return;
        }

        self::process_reconciliation_batch($offset);
    }
}
